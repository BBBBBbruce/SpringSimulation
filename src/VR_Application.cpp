...
// Purpose: Constructor
//-----------------------------------------------------------------------------
VR_Application::VR_Application(int argc, char* argv[])
...
//-----------------------------------------------------------------------------
bool VR_Application::BInitCompositor()
{
...
void VR_Application::Shutdown()
{
	...
	for (std::vector< VR_CG_OpenGL* >::iterator i = m_vecRenderModels.begin(); i != m_vecRenderModels.end(); i++)
	{
		delete (*i);
	}
...
//-----------------------------------------------------------------------------
void VR_Application::ProcessVREvent(const vr::VREvent_t& event)
{
...
//-----------------------------------------------------------------------------
void VR_Application::SetupScene()
{
	if (!m_pHMD)
		return;
...
//-----------------------------------------------------------------------------
void VR_Application::RenderControllerAxes()
{
...
//-----------------------------------------------------------------------------
bool VR_Application::CreateFrameBuffer(int nWidth, int nHeight, FramebufferDesc& framebufferDesc)
{
...
//-----------------------------------------------------------------------------
void VR_Application::RenderScene(vr::Hmd_Eye nEye)
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
//-----------------------------------------------------------------------------
Matrix4 VR_Application::GetHMDMatrixProjectionEye(vr::Hmd_Eye nEye)
{
	if (!m_pHMD)
		return Matrix4();
...
//-----------------------------------------------------------------------------
Matrix4 VR_Application::GetHMDMatrixPoseEye(vr::Hmd_Eye nEye)
{
	if (!m_pHMD)
		return Matrix4();
...
//-----------------------------------------------------------------------------
Matrix4 VR_Application::GetCurrentViewProjectionMatrix(vr::Hmd_Eye nEye)
{
	Matrix4 matMVP;
	if (nEye == vr::Eye_Left)
	{
...
//-----------------------------------------------------------------------------
VR_CG_OpenGL* VR_Application::FindOrLoadRenderModel(const char* pchRenderModelName)
{
	VR_CG_OpenGL* pRenderModel = NULL;
	for (std::vector< VR_CG_OpenGL* >::iterator i = m_vecRenderModels.begin(); i != m_vecRenderModels.end(); i++)
	{
	...
	if (!pRenderModel)
	{
		...
		pRenderModel = new VR_CG_OpenGL(pchRenderModelName);
...
//-----------------------------------------------------------------------------
Matrix4 VR_Application::ConvertSteamVRMatrixToMatrix4(const vr::HmdMatrix34_t& matPose)
{
	Matrix4 matrixObj(
